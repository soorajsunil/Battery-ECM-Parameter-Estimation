function [theta_hat] = fitRelaxationRC(vbatt, ibatt, t, model)
...
switch model
    case '1RC'
        tau1           = LS1_1RC(vrest, trest);                    % Estimate tau1 from rest
        [OCV,R0,R1,C1] = LS2_1RC(vbatt, ibatt, t, tau1);           % Estimate parameters
        theta_hat      = [OCV, R0, R1, C1];                        % Output vector

    case '2RC'
        [tau1, tau2]         = LS1_2RC(vrest, trest);              % Estimate two time constants
        [OCV,R0,R1,C1,R2,C2] = LS2_2RC(vbatt, ibatt, t, tau1, tau2);
        theta_hat            = [OCV, R0, R1, C1, R2, C2];

    case '3RC'
        [tau1, tau2, tau3]         = LS1_3RC(vrest, trest);        % Estimate three time constants
        [OCV,R0,R1,C1,R2,C2,R3,C3] = LS2_3RC(vbatt, ibatt, t, tau1, tau2, tau3);
        theta_hat                  = [OCV, R0, R1, C1, R2, C2, R3, C3];
end
...
function [OCV, R0, R1, C1] = LS2_1RC(vbatt, ibatt, t, tau1)
I  = cumtrapz(t, ibatt);                      % ∫i(t) dt
i1 = currentRC(ibatt, t, tau1);               % RC branch current
M     = [ibatt, ones(size(vbatt)), I, i1];    % Model matrix
theta = M \ vbatt;                            % Least-squares fit
R0  = theta(1);                               % Series resistance
OCV = theta(2);                               % Open-circuit voltage
...
function [OCV,R0,R1,C1,R2,C2] = LS2_2RC(vbatt, ibatt, t, tau1, tau2)
I  = cumtrapz(t, ibatt);                      % ∫i(t) dt
i1 = currentRC(ibatt, t, tau1);               % First RC branch current
i2 = currentRC(ibatt, t, tau2);               % Second RC branch current
M     = [ones(size(vbatt)), ibatt, I, i1, i2];% Model matrix
theta = M \ vbatt;                            % Least-squares fit
OCV = theta(1);                               % Open-circuit voltage
R0  = theta(2);                               % Series resistance
...
function [OCV,R0,R1,C1,R2,C2,R3,C3] = LS2_3RC(vbatt, ibatt, t, tau1, tau2, tau3)
I  = cumtrapz(t, ibatt);                      % ∫i(t) dt
i1 = currentRC(ibatt, t, tau1);               % First RC branch current
i2 = currentRC(ibatt, t, tau2);               % Second RC branch current
i3 = currentRC(ibatt, t, tau3);               % Third RC branch current
M     = [ones(size(vbatt)), ibatt, I, i1, i2, i3]; % Model matrix
theta = M \ vbatt;                            % Least-squares fit
OCV = theta(1);                               % Open-circuit voltage
R0  = theta(2);                               % Series resistance
R1  = theta(4);                               % First RC resistance
...
for k = 2:length(ibatt)
    del     = t(k) - t(k-1);                  % Time step
    i1(k)   = exp(-del / tau) * i1(k-1) + (1 - exp(-del / tau)) * ibatt(k); % Recursive filter
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
